...
class DriveAction:
    def __init__(self):
        ...
        if WaypointMode == True:
            ...
            rospy.loginfo('Waiting for follow_waypoints server'.format(rospy.get_name()))
            ...
            if (self.act_client.wait_for_server(timeout=rospy.Duration.from_sec(5))): # wait for server start up
                ...
                rospy.logerr('[ {} ]: Timedout waiting for selected Server!'.format(rospy.get_name()))     
                err_flag = True  
        else:
            self.status_flag = False # used to throttle further message sending after action execution
    ...
    def followWP(self, data):
        ...
        if (data.action == 'drive'):
            ...
            goal.Xwaypoints = data.Xwaypoints
            goal.Ywaypoints = data.Ywaypoints
